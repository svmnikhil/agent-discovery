...
describe('@agent-discovery chat handler', () => {
  ...
  describe('/assemble command', () => {
    it('writes a starter apm.yml when no agents installed and no recommendations', async () => {
      const s = makeStream();
      await handler(makeRequest('assemble', ''), makeContext(), s.stream as any, makeToken());
      const md = s.markdown();

      expect(md).toContain('```yaml');
      expect(md).toContain('dependencies:');
      // file is written to disk
      const apmPath = join(tempDir, 'apm.yml');
      expect(existsSync(apmPath)).toBe(true);
      const yaml = readFileSync(apmPath, 'utf-8');
      expect(yaml).toContain('apm: []');
      expect(yaml).toContain('Generated by @agent-discovery /assemble');
    });

    it('includes installed catalog entries in apm.yml', async () => {
      const results = searchCatalog('code review', 'agent', 1);
      const targetEntry = results[0].entry;
      ...
      const yaml = readFileSync(join(tempDir, 'apm.yml'), 'utf-8');
      expect(yaml).toContain('github/awesome-copilot');
      expect(yaml).toContain('Installed locally');
    });

    it('includes /review recommendations from the sidecar', async () => {
      // Look up a real catalog entry to use as the recommendation
      const results = searchCatalog('react', 'all', 1);
      const recommendedEntry = results[0].entry;

      // Simulate /review having written the sidecar
      const sidecarDir = join(tempDir, 'docs', 'codebase');
      require('fs').mkdirSync(sidecarDir, { recursive: true });
      require('fs').writeFileSync(
        join(sidecarDir, 'recommended.txt'),
      ...
      const yaml = readFileSync(join(tempDir, 'apm.yml'), 'utf-8');
      expect(yaml).toContain('Recommended by /review');
      expect(yaml).toContain(recommendedEntry.fileName);
      expect(s.markdown()).toMatch(/0 installed locally, 1 from `\/review`/);
    });

    it('dedupes when an entry is both installed AND in the sidecar (installed wins)', async () => {
      const results = searchCatalog('code review', 'agent', 1);
      const entry = results[0].entry;

      const agentsDir = join(tempDir, '.claude', 'agents');
      require('fs').mkdirSync(agentsDir, { recursive: true });
      const destFileName = entry.fileName.replace(/\.agent\.md$/, '.md');
      require('fs').writeFileSync(join(agentsDir, destFileName), '# agent', 'utf-8');

      const sidecarDir = join(tempDir, 'docs', 'codebase');
      require('fs').mkdirSync(sidecarDir, { recursive: true });
      require('fs').writeFileSync(join(sidecarDir, 'recommended.txt'), entry.id + '\n', 'utf-8');
      ...
      const occurrences = (yaml.match(new RegExp(entry.fileName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g')) || []).length;
      expect(occurrences).toBe(1);
      expect(s.markdown()).toMatch(/1 installed locally, 0 from/);
    });
...
`;
      ...
      // recommendations sidecar written for /assemble to pick up
      const sidecarPath = join(tempDir, 'docs', 'codebase', 'recommended.txt');
      expect(existsSync(sidecarPath)).toBe(true);
      const sidecar = readFileSync(sidecarPath, 'utf-8');
      const ids = sidecar.split('\n').filter(l => l && !l.startsWith('#'));
...
`;
  ...
  // ── Welcome lists /review and /assemble ───────────────────────────────────

  it('empty-prompt welcome mentions /review and /assemble', async () => {
    const s = makeStream();
    await handler(makeRequest(undefined, ''), makeContext(), s.stream as any, makeToken());
    const md = s.markdown();
