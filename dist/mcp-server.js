...
import { searchAgents, broadSearch, findByName, findByPartialName, getCatalogInfo, getDuplicateClusters, } from "./catalog.js";
...
// ─── Tool: search_agents ────────────────────────────────────────────
server.tool("search_agents", "Search the local catalog for agents, instructions, or skills using full-text search. No network required.", {
    ...
    try {
        ...
        const text = results
            ...
            if (r._alternatives?.length) {
                const alts = r._alternatives
...
// ─── Tool: recommend ────────────────────────────────────────────────
server.tool("recommend", "Broad catalog search returning up to 30 candidates with descriptions for LLM re-ranking. Use this when making recommendations based on project context.", {
...
server.tool("get_agent_details", "Get full details for a specific catalog entry, including its content fetched from raw.githubusercontent.com.", {
...
async function getDetailsForEntry(entry) {
    try {
...
// ─── Tool: download_agent ───────────────────────────────────────────
server.tool("download_agent", "Download and install a catalog entry to the project. Routes by type: agents → .claude/agents/, instructions → .github/instructions/, skills → .claude/skills/. Auto-converts awesome-copilot agents to Claude Code subagent format.", {
...
// ─── Tool: catalog_info ─────────────────────────────────────────────
server.tool("catalog_info", "Show catalog metadata: version, entry count by type, source list, and build date. Use stats=true to show duplicate clusters across sources.", {
    stats: z
        .boolean()
        .optional()
        .default(false)
