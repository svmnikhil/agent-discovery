...
export async function openCatalog() {
    ...
    if (!fs.existsSync(dbPath)) {
        throw new Error(`Catalog database not found at ${dbPath}. Run the build-catalog script first.`);
    }
...
async function getAllEntries() {
    ...
    const results = catalog.exec(`SELECT id, source, name, type, description, url, filename, tools, tags, fetched_at FROM agents`);
 ...
 */
function relevanceScore(queryTokens, nameTokens, descTokens, tagTokens, toolTokens) {
    let score = 0;
...
function dedupResults(results) {
    ...
    for (let i = 0; i < results.length; i++) {
        for (let j = i + 1; j < results.length; j++) {
            ...
            if (results[i].entry.source === results[j].entry.source)
                continue;
            ...
            if (sim <= JACCARD_THRESHOLD) {
                ...
                const extractSignificantTokens = (name) => {
 ...
 */
export async function searchAgents(query, type, limit = 20) {
 ...
 */
export async function searchWithMeta(query, type, limit = 20) {
    ...
    const scored = entries.map((entry) => {
        ...
        const score = relevanceScore(queryTokens, nameTokens, descTokens, tagTokens, toolTokens);
        return { entry, score };
    });
 ...
 */
export async function broadSearch(query, limit = 30) {
...
export async function findByName(name) {
    const catalog = await openCatalog();
    const results = catalog.exec(`SELECT id, source, name, type, description, url, filename, tools, tags, fetched_at
     FROM agents WHERE lower(name) = lower(?) LIMIT 1`, [name]);
    if (!results.length || !results[0].values.length)
        return null;
 ...
 */
export async function findByPartialName(name) {
    const catalog = await openCatalog();
    const results = catalog.exec(`SELECT id, source, name, type, description, url, filename, tools, tags, fetched_at
     FROM agents WHERE lower(name) LIKE ? LIMIT 10`, [`%${name.toLowerCase()}%`]);
    if (!results.length || !results[0].values.length)
        return [];
    return results[0].values.map(rowToEntry);
}
...
export async function getCatalogInfo() {
    ...
    // Get type counts
    const typeRows = catalog.exec("SELECT type, COUNT(*) as cnt FROM agents GROUP BY type");
    const types = {};
    ...
    const sourceRows = catalog.exec("SELECT DISTINCT source FROM agents ORDER BY source");
    const sources = [];
...
function rowToEntry(row) {
    return {
        ...
        url: row[5],
        fileName: row[6],
        tools: row[7] ? JSON.parse(row[7]) : undefined,
        tags: row[8] ? JSON.parse(row[8]) : undefined,
        fetchedAt: row[9],
    };
}
